...
def do_dielectric_tensor ( data_controller, ene, kramerskronig ):
  ...
  for ispin in range(attributes['nspin']):
    for n in range(d_tensor.shape[0]):
      ...
      epsi, epsr, jdos = do_epsilon(data_controller, ene, kramerskronig, ispin, ipol, jpol)
...
def do_epsilon ( data_controller, ene, kramerskronig, ispin, ipol, jpol ):
